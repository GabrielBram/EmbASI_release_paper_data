...
def run_single_fragment(atoms, embed_mask=None, hl_calc=calc_hl, ll_calc=calc_ll, hl_xc="pbe", ll_xc="pbe", post_hf_method=None, basis="N/A", trunc=None, run_dir="./EmbASI_calc", frag_name="Default"):
    ...
    if trunc is None:
        ...
        Projection = ProjectionEmbedding(atoms, embed_mask=embed_mask, calc_base_ll=ll_calc, calc_base_hl=hl_calc, mu_val=1.e+6, gc=True, parallel=True, projection="huzinaga-sc", run_dir=run_dir, localisation="SPADE")
    else:
        run_dir = os.path.join(run_dir, str(trunc))
        Projection = ProjectionEmbedding(atoms, embed_mask=embed_mask, calc_base_ll=ll_calc, calc_base_hl=hl_calc, mu_val=1.e+6, gc=True, parallel=True, projection="huzinaga-sc", run_dir=run_dir, localisation="SPADE", truncate_basis_thresh=trunc)
