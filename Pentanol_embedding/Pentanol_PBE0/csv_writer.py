...
def run_single_fragment_full_molecule(atoms, hl_calc=calc_hl, ll_calc=calc_ll, hl_xc="pbe", ll_xc="pbe", post_hf_method=None, basis="N/A", run_dir="./Embasi_calc"):
    ...
    Projection = StandardDFT(atoms, calc_base_ll=ll_calc, calc_base_hl=ll_calc, run_dir=run_dir)
    Projection.run()

    data_dict = {}
    data_dict["LL XC"] = ll_calc.parameters["xc"]
    data_dict["HL XC"] = hl_calc.parameters["xc"]
    data_dict["HL Atoms"] = len(atoms)
    if post_hf_method is not None:
...
def run_single_fragment(atoms, embed_mask=None, hl_calc=calc_hl, ll_calc=calc_ll, hl_xc="pbe", ll_xc="pbe", post_hf_method=None, basis="N/A", run_dir="./EmbASI_calc"):
    ...
    Projection = ProjectionEmbedding(atoms, embed_mask=embed_mask, calc_base_ll=ll_calc, calc_base_hl=hl_calc, mu_val=1.e+6, gc=True, parallel=True, projection="huzinaga", run_dir=run_dir, localisation="qmcode")
    Projection.run()
